document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.js-role-select').forEach(select => {
        select.addEventListener('change', function() {
            ...
            fetch('index.php?page=admin_user_role', {
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
    ...
    if (btnFilter) {
        function loadStats() {
            ...
            fetch('index.php?page=admin_stats_filter', {
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
                    ...
                    if (data.status === 'success') {
                        ...
                        if (data.history.length > 0) {
                            data.history.forEach(item => {
                                if (parseInt(item.total) > maxOrders) maxOrders = parseInt(item.total);
                            });
                            data.history.forEach(item => {
    ...
    if (replyForm) {
        replyForm.addEventListener('submit', function(e) {
            ...
            fetch('index.php?page=admin_message_reply', {
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
    ...
    document.querySelectorAll('.js-save-horaire').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            fetch('index.php?page=admin_horaire_update', {
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
    ...
    if (formCreateUser) {
        formCreateUser.addEventListener('submit', function(e) {
            ...
            fetch('index.php?page=admin_user_create', {
                    method: 'POST',
                    body: formData
                })
                .then(res => res.json())
